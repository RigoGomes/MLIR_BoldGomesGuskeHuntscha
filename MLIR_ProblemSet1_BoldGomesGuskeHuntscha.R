...
#1.3. Explain what the following in the summary-outputs mean. 
    ...
    # Std.Error (SE):
      # SE The standard error is an estimate of the standard deviation of the coefficient, i.e. the amount it varies across the sample. 
      ...
      # Age (continuous): The impact of a 1 unit change in age on log(FEV) should be between (0.016691 <= x <= 0.030083) with a 95% probability
      # Therefore, we can say that with (at least) 95% certainty the impact of Age on log(FEV) is positive
      # Smoke (dummy): The impact of smoking (Smoke=1) on log(FEV) should be between (-0.087887 <= x <= -0.004247) with a 95% probability
      # Therefore, we can say that with (at least) 95% certainty the impact of Smoke on log(FEV) is negative
...
#################################################
  ...
  setwd() #set working directory such that tennis dataset is in the same file
  getwd()
...
#2.2 Perform a logistic regression with the full data set and print the numerical results.
    ...
    # Both coefficients for x1 and x2 are highly significant at the 0.1%-level.
...
#4.3 What are the main differences between using adjusted R? for model selection and using cross-validation (with mean squared test error MSE)?
...
#4.4. Use lasso and ridge regression on the data to predict the logprice. Briefly describe the results and compare them to a simple multiple linear regression.
    ...
    ####Ridge####
        ...
        # Ridge regression therefore is a shrinkage method (cf. results of the multiple linear regression model).
        ...
        # This is due to another penalty function (l2 vs. l2 norm).
    ...
    ###Linear model###
        ...
        #The coefficients are larger than in the ridge and lasso regressions. 
        ...
        #The MSE for the linear model is smaller than the one for the LASSO and ridge regression. 
        #This is due to...
    
    
    
    
    ###Mein Vorschlag (Johannes)
